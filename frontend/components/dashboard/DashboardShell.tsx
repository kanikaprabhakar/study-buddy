...
const FLOATERS = [
  { src: "/images/9.png",  w: 42, top: "6%",    left: "3%",   anim: "animate-float-slow",   delay: "0s" },
  { src: "/images/10.png", w: 36, top: "10%",   right: "4%",  anim: "animate-float-medium", delay: "1.3s" },
  { src: "/images/11.png", w: 32, bottom: "16%", left: "2%",  anim: "animate-float-fast",   delay: "0.7s" },
  { src: "/images/12.png", w: 38, bottom: "8%",  right: "3%", anim: "animate-float-slow",   delay: "1.9s" },
  { src: "/images/9.png",  w: 26, top: "48%",   left: "1%",   anim: "animate-float-medium", delay: "2.2s" },
  { src: "/images/10.png", w: 28, top: "62%",   right: "2%",  anim: "animate-float-fast",   delay: "0.4s" },
] as const;
    ...
    <main className="relative min-h-screen overflow-x-hidden animate-gradient-bg">
      ...
      <div className="sticky top-0 z-50 px-4 pt-3 sm:px-6">
        ...
        <Link href="/" className="flex items-center gap-2">
          ...
          />
          <span className="font-display text-lg font-bold italic text-fg-primary">
            Study Buddy
          </span>
        </Link>
      ...
      <div className="relative z-10 mx-auto w-full max-w-5xl px-4 pb-16 sm:px-6">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          ...
          {/* Pomodoro */}
          <DashCard title="Pomodoro Quick Start">
            <p className="text-sm text-fg-secondary">25 min focus / 5 min break</p>
            <button
              type="button"
              className="mt-4 inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-bold text-white shadow-lg transition-all hover:scale-105 hover:shadow-xl animate-pulse-glow"
...
function NavDropdown({ dark }: { dark: boolean }) {
  ...
  return (
    <div ref={ref} className="relative hidden sm:block">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 rounded-xl px-4 py-2 text-sm font-bold transition-all hover:scale-105"
        style={{
          background: open
            ? (dark ? "rgba(203,67,139,0.18)" : "rgba(203,67,139,0.12)")
            : (dark ? "rgba(203,67,139,0.10)" : "rgba(203,67,139,0.07)"),
          color: "var(--fg-primary)",
          border: `1px solid ${open ? "rgba(203,67,139,0.35)" : "rgba(203,67,139,0.18)"}`,
        }}
      >
        Pages
        <svg
          ...
          style={{ transform: open ? "rotate(180deg)" : "none", color: "#CB438B" }}
        >
      ...
      {open && (
        <div
          className="absolute left-0 top-full mt-2 w-44 rounded-2xl border shadow-2xl backdrop-blur-xl overflow-hidden z-50"
          style={{ background: "var(--nav-glass)", borderColor: "var(--nav-border)" }}
        >
          <Link
            href="/tasks"
            onClick={() => setOpen(false)}
            className="flex items-center px-4 py-3 text-sm font-semibold text-fg-primary transition-colors hover:text-[#CB438B]"
            style={{ borderBottom: `1px solid ${dark ? "rgba(203,67,139,0.12)" : "rgba(203,67,139,0.08)"}` }}
          >
            Tasks
          </Link>
          {/* More pages go here */}
